...
function nm_indexnow_plugin_action_links( $links ) {
	...
	$settings_link = sprintf(
		...
		esc_html__( 'Settings', 'nm-indexnow' )
	);
	...
	if ( is_wp_error( $response ) || (int) wp_remote_retrieve_response_code( $response ) >= 400 ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$error_message = sprintf(
				...
				__( 'IndexNow request failed with HTTP status %1$d: %2$s.', 'nm-indexnow' ),
				$code,
				$message
			);
		}
...
function nm_indexnow_admin_notices() {
	...
	<div class="notice notice-error is-dismissible">
		<p>
			<strong><?php esc_html_e( 'IndexNow Integration', 'nm-indexnow' ); ?>:</strong>
			<?php echo esc_html( $message ); ?>
			<a href="<?php echo esc_url( $settings_url ); ?>"><?php esc_html_e( 'Check your IndexNow settings.', 'nm-indexnow' ); ?></a>
		</p>
	</div>
	<?php
}
...
function nm_indexnow_register_settings() {
	...
	add_settings_field(
		'nm_indexnow_api_key',
		__( 'IndexNow API Key', 'nm-indexnow' ),
	...
	add_settings_field(
		'nm_indexnow_key_location',
		__( 'IndexNow Key File', 'nm-indexnow' ),
...
function nm_indexnow_section_cb() {
	$text = __( 'Configure IndexNow integration for your site.', 'nm-indexnow' );
	?>
	<p>
		<?php echo esc_html( $text ); ?> <a href="https://www.indexnow.org/" title="<?php esc_attr_e( 'IndexNow', 'nm-indexnow' ); ?>" target="_blank" rel="noopener noreferrer nofollow"><?php esc_html_e( 'Learn more', 'nm-indexnow' ); ?></a>.
	</p>
	<?php
}
...
function nm_indexnow_api_key_field_cb() {
	...
	<p class="description">
		<?php
		if ( $using_generated ) {
			esc_html_e( 'You are currently using the API key automatically generated by this plugin. You can keep using it, or replace it with your own IndexNow key.', 'nm-indexnow' );
		} else {
			esc_html_e( 'Your IndexNow API key.', 'nm-indexnow' );
		}
		?> <a href="https://www.bing.com/indexnow/getstarted#implementation" title="<?php esc_attr_e( 'Generate a free IndexNow key', 'nm-indexnow' ); ?>" target="_blank" rel="noopener noreferrer nofollow"><?php esc_html_e( 'Get a free key', 'nm-indexnow' ); ?></a>.
	</p>
	<?php
}
...
function nm_indexnow_key_location_field_cb() {
	...
	<label for="nm_indexnow_key_location">
		<input type="checkbox" id="nm_indexnow_key_location" name="nm_indexnow_key_location" value="1" <?php checked( $checked ); ?> /> <?php esc_html_e( 'My IndexNow key file is uploaded in the root folder of my site.', 'nm-indexnow' ); ?>
	</label>
	<p class="description">
		<?php esc_html_e( '(Optional) If checked, your key file URL will be sent to IndexNow as part of each request.', 'nm-indexnow' ); ?>
	</p>
	<?php
	...
	if ( $checked ) {
		...
		if ( $api_key === '' ) {
			...
			<p class="description" style="color:#b32d2e;">
				<?php esc_html_e( 'Key file is enabled, but no API key is set. Save your API key first.', 'nm-indexnow' ); ?>
			</p>
			<?php
		} else {
			$filename  = $api_key . '.txt';
			...
			if ( $file_exists ) {
				...
				<p class="description" style="color:#00a32a;">
					...
					printf(
						...
						esc_html__( 'Key file detected at %s.', 'nm-indexnow' ),
						esc_url( $file_url )
					);
					?>
				</p>
				...
				<p class="description" style="color:#b32d2e;">
					...
					printf(
						/* translators: %s: key file URL */
						esc_html__( 'Key file not found or not readable at %s. Make sure the file exists and contains only your API key.', 'nm-indexnow' ),
						esc_url( $file_url )
					);
					?>
				</p>
